...
def get_output_filepath(output_filename, folder_override=None):
    ...
    Args:
        output_filename (str): The desired output filename (with or without extension)
        folder_override (str, optional): Override the default folder
...
def process_url(url: str, setting: str, keyword: str = "", output_folder: str = DEFAULT_OUTPUT_FOLDER, task_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        if setting.lower() == "pdf":
            ...
            json_output = os.path.join(output_folder, output_json_name + ".json")
            
            # Process if structify module is available
            if structify_module:
                try:
                    # Try direct PDF processing if available
                    if hasattr(structify_module, 'process_pdf'):
                        # Detect document type to determine OCR needs
                        doc_type = None
            ...
            return {
                ...
                "url": url,
                "pdf_file": pdf_file,
                "json_file": json_output if structify_module else None,
                "setting": setting,
            ...
            json_path = convert_to_json(text, url, output_folder)
            ...
            metadata = {
                ...
                "canonical_url": soup.find('link', attrs={'rel': 'canonical'})['href'] if soup.find('link', attrs={'rel': 'canonical'}) else url,
                "h1_tags": [h1.get_text(strip=True) for h1 in soup.find_all('h1')],
            }
            
            # Save metadata to JSON
            filename = hashlib.md5(url.encode()).hexdigest()[:10] + "_metadata.json"
            ...
            # Save title to JSON
            filename = hashlib.md5(url.encode()).hexdigest()[:10] + "_title.json"
            json_path = os.path.join(output_folder, filename)
            ...
            if not keyword:
                return {"status": "error", "url": url, "error": "Keyword required for keyword search setting"}
                
            # Extract text and search for keyword
            text = extract_html_text(url)
            ...
            # Save to JSON
            filename = hashlib.md5(f"{url}_{keyword}".encode()).hexdigest()[:10] + "_keyword.json"
            json_path = os.path.join(output_folder, filename)
            ...
            return {
                ...
                "keyword": keyword,
                "matches_count": len(contexts),
                "contexts": contexts[:5],  # Return first 5 for preview
                "json_file": json_path,
                "setting": setting,
                "processing_time": time.time() - start_time
            }
        ...
        logger.error(f"Error processing URL {url} with setting {setting}: {e}")
        return {
            "status": "error",
            "url": url,
            "error": str(e),
