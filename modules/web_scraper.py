...
DEFAULT_TIMEOUT = int(os.environ.get("PDF_DOWNLOAD_TIMEOUT", "60"))
MAX_RETRIES = int(os.environ.get("PDF_MAX_RETRIES", "5"))
...
def download_pdf(url: str, save_path: str = DEFAULT_OUTPUT_FOLDER, emit_progress=True, task_id=None, timeout=DEFAULT_TIMEOUT) -> str:
    ...
    """
    logger.info(f"DOWNLOAD_DEBUG: Starting download of PDF: {url} to {save_path}")
    ...
    if "arxiv.org/abs/" in url:
        ...
        if not pdf_url.lower().endswith(".pdf"):
            pdf_url += ".pdf"
        logger.info(f"DOWNLOAD_DEBUG: Converted arXiv abstract URL to PDF URL: {pdf_url}")
    else:
        pdf_url = url
        logger.info(f"DOWNLOAD_DEBUG: Using original URL: {pdf_url}")
    
    # Ensure the save directory exists
    try:
        os.makedirs(save_path, exist_ok=True)
        logger.info(f"DOWNLOAD_DEBUG: Ensured download directory exists: {save_path}")
    except Exception as e:
        logger.error(f"DOWNLOAD_DEBUG: Failed to create download directory {save_path}: {e}")
    ...
    logger.info(f"DOWNLOAD_DEBUG: Generated URL hash: {url_hash}")
    
    # Extract filename from the URL
    filename = os.path.basename(pdf_url.split('?')[0])  # Remove query parameters
    logger.info(f"DOWNLOAD_DEBUG: Extracted filename: {filename}")
    
    # Handle special case for empty filename
    if not filename:
        filename = "document.pdf"
        logger.info(f"DOWNLOAD_DEBUG: Empty filename, using default: {filename}")
    
    # Fix: Properly handle files with or without extensions to avoid double extension
    base_name, ext = os.path.splitext(filename)
    if not ext or ext.lower() != '.pdf':
        ext = '.pdf'
    logger.info(f"DOWNLOAD_DEBUG: Base name: {base_name}, extension: {ext}")
    ...
    logger.info(f"DOWNLOAD_DEBUG: Final file path: {file_path}")
    ...
    if os.path.exists(file_path) and os.path.getsize(file_path) > 1000:  # Ensure it's not an empty file
        logger.info(f"DOWNLOAD_DEBUG: PDF already exists: {file_path}")
    ...
    for attempt in range(max_retries):
        try:
            logger.info(f"DOWNLOAD_DEBUG: Download attempt {attempt+1}/{max_retries} for {pdf_url}")
            
            # Use optimized request headers to prevent compression/encoding issues
            download_headers = {
                "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36",
                "Accept": "application/pdf,*/*",
                "Connection": "keep-alive",
                "Accept-Encoding": "identity",  # Prevent any compression that might cause issues
                "Cache-Control": "no-cache, no-store"  # Prevent caching
            }
            logger.info(f"DOWNLOAD_DEBUG: Request headers: {download_headers}")
            
            logger.info(f"DOWNLOAD_DEBUG: Sending GET request with timeout {timeout}s")
            ...
            logger.info(f"DOWNLOAD_DEBUG: Response status code: {response.status_code}")
            response.raise_for_status()
            
            # Get content length for progress tracking
            content_length = int(response.headers.get('Content-Length', 0))
            logger.info(f"DOWNLOAD_DEBUG: Content-Length: {content_length}")
            
            # Get content type
            content_type = response.headers.get('Content-Type', '').lower()
            logger.info(f"DOWNLOAD_DEBUG: Content-Type: {content_type}")
            ...
            if content_type and 'application/pdf' not in content_type and not pdf_url.lower().endswith('.pdf'):
                logger.warning(f"DOWNLOAD_DEBUG: Content type not PDF: {content_type}. Checking content...")
                
                # Check first few bytes to see if it starts with %PDF
                first_chunk = next(response.iter_content(256), None)
                if first_chunk:
                    logger.info(f"DOWNLOAD_DEBUG: First bytes: {first_chunk[:20]}")
                    
                if not first_chunk or not first_chunk.startswith(b'%PDF-'):
                    logger.error(f"DOWNLOAD_DEBUG: Content is not a PDF: {first_chunk[:20] if first_chunk else 'empty'}")
                    if emit_progress:
                        emit_socket_event("pdf_download_progress", {
                            "task_id": task_id,
                            "url": url,
            ...
            # Try alternative download method for retry attempts
            if attempt > 0:  # On retry attempts, try an alternative download method
                logger.info(f"DOWNLOAD_DEBUG: Using alternative in-memory download method on retry attempt {attempt+1}")
                
                # Download in memory first, then write to file in one operation
                try:
                    pdf_content = b''
                        ...
                        if chunk:
                            ...
                            if emit_progress and content_length > 0:
                                ...
                                if progress >= last_progress_update + 5 or progress >= 99:
                                    logger.info(f"DOWNLOAD_DEBUG: Download progress: {progress}% ({downloaded_bytes}/{content_length})")
                                    emit_socket_event("pdf_download_progress", {
                                        "task_id": task_id,
                                        "url": url,
                                        "progress": progress,
                    ...
                    # Verify content is a PDF before writing
                    ...
                    if os.path.getsize(file_path) != len(pdf_content):
                        raise ValueError(f"File size mismatch: {os.path.getsize(file_path)} vs {len(pdf_content)}")
                        
                    logger.info(f"DOWNLOAD_DEBUG: Alternative download method successful: {file_path}")
                    ...
                    if emit_progress:
                        emit_socket_event("pdf_download_progress", {
                            ...
                            "progress": 100,
                            "status": "success",
                            "message": "Download complete (alternative method)",
                            "file_path": file_path
                        })
                    
                    return file_path
                except Exception as e:
                    logger.warning(f"DOWNLOAD_DEBUG: Alternative download method failed: {e}")
                    # Continue with the standard method
            
            # Try low-level OS file writing on last attempt
            if attempt >= max_retries - 1:  # Last attempt
                try:
                    logger.info(f"DOWNLOAD_DEBUG: Using low-level file writing on final attempt")
                    ...
                    # Write using low-level file operations
                    fd = os.open(file_path, os.O_WRONLY | os.O_CREAT | os.O_TRUNC | (os.O_BINARY if hasattr(os, 'O_BINARY') else 0))
                    try:
                        os.write(fd, response_content)
                        os.fsync(fd)
                    finally:
                        os.close(fd)
                        
                    # Verify file was written correctly
                    if os.path.getsize(file_path) != len(response_content):
                        raise ValueError(f"File size mismatch: {os.path.getsize(file_path)} vs {len(response_content)}")
                        
                    logger.info(f"DOWNLOAD_DEBUG: Low-level file writing successful: {file_path}")
                    ...
                    if emit_progress:
                        emit_socket_event("pdf_download_progress", {
                            ...
                            "progress": 100,
                            "status": "success",
                            "message": "Download complete (low-level method)",
                            "file_path": file_path
                        })
                    
                    return file_path
                except Exception as e:
                    logger.warning(f"DOWNLOAD_DEBUG: Low-level file writing failed: {e}")
                    # Continue with standard approach
            
            # Standard streaming download
            downloaded_bytes = 0
            last_progress_update = 0
            logger.info(f"DOWNLOAD_DEBUG: Starting to stream content to file: {file_path}")
            
            with open(file_path, "wb") as f:
                # Flush directory entry to ensure file exists
                os.fsync(os.path.dirname(file_path))
                
                # Use a smaller chunk size with explicit flushing
                for chunk in response.iter_content(chunk_size=8192):
                    if chunk:
                        # Write and immediately flush to ensure content is written to disk
                        f.write(chunk)
            ...
            # Force a file system sync to ensure all data is written
            try:
                os.fsync(f.fileno())
            except Exception as e:
                logger.warning(f"DOWNLOAD_DEBUG: Could not fsync file: {e}")
            
            logger.info(f"DOWNLOAD_DEBUG: Download completed, checking file: {file_path}")
            # Verify the file is a valid PDF and has content
            if not os.path.exists(file_path):
                logger.error(f"DOWNLOAD_DEBUG: File doesn't exist after download: {file_path}")
                if emit_progress:
                    emit_socket_event("pdf_download_progress", {
                        "task_id": task_id,
                        "url": url,
            ...
            logger.info(f"DOWNLOAD_DEBUG: File size after download: {file_size} bytes")
            
            if file_size < 1000:
                logger.error(f"DOWNLOAD_DEBUG: File too small, probably not a valid PDF: {file_size} bytes")
                if os.path.exists(file_path):
                    os.remove(file_path)
                    logger.info(f"DOWNLOAD_DEBUG: Removed invalid file: {file_path}")
            ...
            try:
                with open(file_path, 'rb') as f:
                    ...
                    if not header.startswith(b'%PDF-'):
                        logger.error(f"DOWNLOAD_DEBUG: File doesn't have PDF header: {header[:20]}")
                        os.remove(file_path)
                        ...
                        if emit_progress:
                            emit_socket_event("pdf_download_progress", {
                                ...
                                "url": url,
                                "status": "error",
                                "message": "Downloaded file is not a valid PDF (invalid header)"
                            })
                        
                        if attempt < max_retries - 1:
                            continue
                        else:
                            raise ValueError("Downloaded file is not a valid PDF (invalid header)")
                    else:
                        logger.info(f"DOWNLOAD_DEBUG: Valid PDF header confirmed: {header[:20]}")
            except Exception as e:
                logger.error(f"DOWNLOAD_DEBUG: Error checking PDF header: {e}")
                if attempt < max_retries - 1:
                    continue
                else:
                    raise
                
            logger.info(f"DOWNLOAD_DEBUG: PDF successfully downloaded to: {file_path}")
            ...
            logger.warning(f"DOWNLOAD_DEBUG: Attempt {attempt+1}/{max_retries} timed out: {e}")
            ...
            if attempt < max_retries - 1:
                # Exponential backoff
                delay = (2 ** attempt) * 1.5
                logger.info(f"DOWNLOAD_DEBUG: Retrying in {delay:.1f} seconds...")
                time.sleep(delay)
            else:
                logger.error(f"DOWNLOAD_DEBUG: Failed to download PDF after {max_retries} timeout attempts")
            ...
            logger.warning(f"DOWNLOAD_DEBUG: Attempt {attempt+1}/{max_retries} failed: {e}")
            logger.warning(f"DOWNLOAD_DEBUG: Exception type: {type(e).__name__}")
            logger.warning(f"DOWNLOAD_DEBUG: Traceback: {traceback.format_exc()}")
            ...
            if attempt < max_retries - 1:
                ...
                logger.error(f"DOWNLOAD_DEBUG: Failed to download PDF after {max_retries} attempts: {e}")
...
class BatchWebScraper:
    ...
    def _process_url_with_tracking(self, url, setting, keyword, output_folder):
        ...
        try:
            # Special handling for PDF downloads with better error tracking
            if setting.lower() == "pdf":
                logger.info(f"Processing PDF URL: {url} with enhanced error handling")
                
                try:
                    # Try downloading with explicit logging
                    ...
                    if not pdf_file or not os.path.exists(pdf_file):
                        logger.error(f"PDF download failed - file not found: {url}")
                        return {"error": "Downloaded PDF not found", "url": url, "status": "error"}
                    
                    file_size = os.path.getsize(pdf_file)
                    logger.info(f"PDF download verification - file size: {file_size} bytes")
                    
                    if file_size < 1000:
                        logger.error(f"PDF too small ({file_size} bytes): {pdf_file}")
                        if os.path.exists(pdf_file):
                            os.remove(pdf_file)
                        return {"error": "Downloaded PDF is too small (truncated)", "url": url, "status": "error"}
                    
                    # Track in PDF downloads
                    with self.pdf_downloads_lock:
                        self.pdf_downloads.append({
                            "url": url,
                            "filePath": pdf_file,
                    ...
                    logger.error(f"Error in PDF download process: {pdf_err}")
                    return {"error": str(pdf_err), "url": url, "status": "error"}
            
            # For non-PDF settings, use the standard process_url function
