...
function smoothProgress(taskId, reportedProgress, updateCount) {
  // If task is in early stages (below 15%) and has received multiple updates
  if (reportedProgress <= DEFAULT_SETTINGS.lowProgressThreshold && updateCount > 2) {
    // Calculate a smoothed progress percentage, increasing gradually with update count
    // This helps prevent the "stuck at 5%" perception
    const minProgress = Math.max(reportedProgress, 1); // Ensure at least 1%
    const additionalProgress = Math.min(10, updateCount - 2);
    const smoothedProgress = Math.min(
      DEFAULT_SETTINGS.lowProgressThreshold, 
      minProgress + additionalProgress
    );
