...
const playlistDownloader = {
  ...
  async handlePlaylistSubmit(e) {
    ...
    try {
      ...
      this.updateProgressDisplay(0, "Initializing...");
  ...
  /**
   * Get detailed playlist stats
   * @param {string} taskId - Task ID
   * @returns {Promise<Object>} - Playlist statistics
   */
  async getPlaylistStats(taskId) {
    if (!taskId) return null;
    
    try {
      // Try to get stats from current state first
      if (state.taskData && state.taskData.stats) {
        return this.processStatsData(state.taskData.stats);
      }
      
      // Try to fetch from API if not in state
      const status = await this.fetchTaskStatus(taskId);
      if (status && status.stats) {
        return this.processStatsData(status.stats);
      }
  ...
  /**
   * Fix progress bar styling for consistency
   * @param {HTMLElement} progressBar - Progress bar element
   * @param {number} progress - Progress percentage
   */
  fixProgressBarStyling(progressBar, progress) {
    if (!progressBar) return;
    ...
    // Add appropriate class based on progress percentage
    if (progress >= 100) {
      progressBar.classList.add('bg-success');
    } else if (progress >= 75) {
      progressBar.classList.add('bg-info');
    ...
    // Add pulsing animation for very low progress to indicate activity
    const progressContainer = progressBar.closest('.progress');
    if (progressContainer) {
      if (progress > 0 && progress < 10) {
        progressContainer.classList.add('progress-pulsing');
      } else {
        progressContainer.classList.remove('progress-pulsing');
      }
    }
  },

  /**
   * Set up Socket.IO events specifically for playlist processing
   * @param {string} taskId - Task ID
   */
  setupSocketEvents(taskId) {
    if (!window.socket) {
      console.warn("Socket.IO not available for event setup");
      return;
    }
    
    try {
      console.log("Setting up playlist-specific Socket.IO events");
      
      // Clear existing handlers to prevent duplicates
      const playlistEvents = [
        'playlist_progress', 'playlist_completed', 'playlist_error', 'playlist_cancelled',
        'video_download_started', 'video_download_completed', 'video_download_error'
      ];
      
      // Remove existing event handlers
      playlistEvents.forEach(event => window.socket.off(event));
      
      // Set up playlist-specific progress updates
      window.socket.on('playlist_progress', (data) => {
        if (data && data.task_id === taskId) {
          this.processPlaylistProgressUpdate(data);
        }
      });
      
      // Video-specific events
      window.socket.on('video_download_started', (data) => {
        if (data && data.task_id === taskId) {
          this.handleVideoDownloadStarted(data);
        }
      });
      
      window.socket.on('video_download_completed', (data) => {
        if (data && data.task_id === taskId) {
          this.handleVideoDownloadCompleted(data);
        }
      });
      
      window.socket.on('video_download_error', (data) => {
        if (data && data.task_id === taskId) {
          this.handleVideoDownloadError(data);
        }
      });
      ...
      console.log("Playlist-specific Socket.IO events setup complete");
    } catch (error) {
  ...
  /**
   * Process progress update specific to playlists
   * @param {Object} data - Progress update data
   */
  processPlaylistProgressUpdate(data) {
    try {
      // First process with standard handler
      this.processStatusUpdate(data);
      
      // Then handle playlist-specific data
      if (data.current_stage) {
        this.updateStageProgress(data.current_stage, data.message);
      }
      
      // Update video progress if available
      if (data.current_video) {
        this.updateCurrentVideoProgress(data.current_video, data.stage_progress || 0);
      }
    } catch (error) {
      console.error("Error processing playlist progress update:", error);
    }
  },

  /**
   * Update progress display for the current stage
   * @param {string} stage - Current processing stage
   * @param {string} message - Status message
   */
  updateStageProgress(stage, message) {
    try {
      // Map of stages to user-friendly labels
      const stageLabels = {
        'init': 'Initializing playlist processing',
        'initialization': 'Starting playlist download',
        'retrieving_videos': 'Retrieving playlist videos',
        'video_ids': 'Gathering video IDs',
        'titles': 'Retrieving video titles',
        'retrieving_titles': 'Getting video information',
        'download': 'Downloading transcripts',
        'downloading_transcripts': 'Extracting transcripts',
        'json_processing': 'Processing transcripts',
        'finalizing': 'Generating JSON output',
        'complete': 'Finishing playlist processing'
      };
      
      // Get or create stage display element
      let stageDisplay = document.getElementById('playlist-stage-display');
      if (!stageDisplay) {
        // Try to find a good container for this
        const progressStatusContainer = document.querySelector('.progress-status-container') || 
                                      document.getElementById('playlist-progress-status')?.parentNode;
        
        if (progressStatusContainer) {
          stageDisplay = document.createElement('div');
          stageDisplay.id = 'playlist-stage-display';
          stageDisplay.className = 'small text-muted mt-1';
          progressStatusContainer.appendChild(stageDisplay);
        }
      }
      
      // Update the display
      if (stageDisplay) {
        stageDisplay.textContent = stageLabels[stage] || message || `Processing stage: ${stage}`;
      }
    } catch (error) {
  ...
  /**
   * Update progress for current video being processed
   * @param {string} videoTitle - Title of video being processed
   * @param {number} progress - Progress percentage for this video
   */
  updateCurrentVideoProgress(videoTitle, progress) {
    try {
      // Get or create video progress container
      let videoProgress = document.getElementById('current-video-progress');
      if (!videoProgress) {
        const progressStatsContainer = document.getElementById('playlist-progress-stats');
        if (progressStatsContainer) {
          const videoContainer = document.createElement('div');
          videoContainer.className = 'current-video-container mt-2 border-top pt-2';
          videoContainer.innerHTML = `
            <div class="d-flex align-items-center justify-content-between">
              <div class="text-truncate" id="current-video-title"></div>
              <span class="badge bg-info" id="current-video-progress-percent">0%</span>
            </div>
            <div class="progress mt-1" style="height: 4px;">
              <div id="current-video-progress" class="progress-bar bg-info" 
                  role="progressbar" style="width: 0%" 
                  aria-valuenow="0" aria-valuemin="0" aria-valuemax="100"></div>
            </div>
          `;
          progressStatsContainer.appendChild(videoContainer);
          
          videoProgress = document.getElementById('current-video-progress');
        }
      }
      
      // Update video title and progress
      if (videoProgress) {
        const videoTitleElement = document.getElementById('current-video-title');
        const videoProgressPercent = document.getElementById('current-video-progress-percent');
        
        if (videoTitleElement) {
          videoTitleElement.textContent = videoTitle;
          videoTitleElement.title = videoTitle; // For tooltip on hover
        }
        
        if (videoProgressPercent) {
          videoProgressPercent.textContent = `${Math.round(progress * 100)}%`;
        }
        
        // Update progress bar
        videoProgress.style.width = `${Math.round(progress * 100)}%`;
        videoProgress.setAttribute('aria-valuenow', Math.round(progress * 100));
      }
    } catch (error) {
      console.error("Error updating current video progress:", error);
    }
  },

  /**
   * Handle video download started event
   * @param {Object} data - Video download data
   */
  handleVideoDownloadStarted(data) {
    try {
      // Update stats display with current video
      if (data.video && data.video.title) {
        this.updateCurrentVideoProgress(data.video.title, 0);
      }
      
      console.log(`Started downloading video: ${data.video?.title || 'Unknown'}`);
    } catch (error) {
      console.error("Error handling video download started:", error);
    }
  },

  /**
   * Handle video download completed event
   * @param {Object} data - Video download data
   */
  handleVideoDownloadCompleted(data) {
    try {
      // Update stats display
      if (data.video && data.video.title) {
        this.updateCurrentVideoProgress(data.video.title, 1.0); // 100% complete
        
        // Show a brief success indicator
        setTimeout(() => {
          // Try to find the video container and add a completion badge
          const videoTitleElement = document.getElementById('current-video-title');
          if (videoTitleElement) {
            videoTitleElement.innerHTML = `<i class="fas fa-check-circle text-success me-1"></i> ${data.video.title}`;
          }
        }, 300);
      }
      
      console.log(`Completed downloading video: ${data.video?.title || 'Unknown'}`);
    } catch (error) {
      console.error("Error handling video download completed:", error);
    }
  },

  /**
   * Handle video download error event
   * @param {Object} data - Video download error data
   */
  handleVideoDownloadError(data) {
    try {
      // Update stats display
      if (data.video && data.video.title) {
        // Try to find the video container and add an error indicator
        const videoTitleElement = document.getElementById('current-video-title');
        if (videoTitleElement) {
          videoTitleElement.innerHTML = `<i class="fas fa-times-circle text-danger me-1"></i> ${data.video.title}`;
          videoTitleElement.title = `Error: ${data.error || 'Unknown error'}`;
        }
        
        // Update progress bar to show error
        const videoProgress = document.getElementById('current-video-progress');
        if (videoProgress) {
          videoProgress.classList.remove('bg-info', 'bg-success');
          videoProgress.classList.add('bg-danger');
        }
      }
      
      console.error(`Error downloading video: ${data.video?.title || 'Unknown'}`, data.error);
    } catch (error) {
      console.error("Error handling video download error:", error);
    }
  },
  /**
   * Show toast notification
...
export const loadDependenciesIfNeeded = playlistDownloader.loadDependenciesIfNeeded.bind(playlistDownloader);
export const handleSocketProgressUpdate = playlistDownloader.handleSocketProgressUpdate.bind(playlistDownloader);
export const handleSocketTaskCompleted = playlistDownloader.handleSocketTaskCompleted.bind(playlistDownloader);
export const handleSocketTaskError = playlistDownloader.handleSocketTaskError.bind(playlistDownloader);
export const handleSocketTaskCancelled = playlistDownloader.handleSocketTaskCancelled.bind(playlistDownloader);
export const addUrlField = playlistDownloader.addUrlField.bind(playlistDownloader);
