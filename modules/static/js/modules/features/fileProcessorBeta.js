...
// Import utility modules - with proper error handling for each
...
const fileProcessor = {
  ...
  /**
   * Process status update from server (via socket or polling)
  ...
  processStatusUpdate(data) {
    try {
      ...
      // Update UI using progress handler if available
      if (state.progressHandler && typeof state.progressHandler.updateProgress === 'function') {
        state.progressHandler.updateProgress(
          data.progress || 0,
