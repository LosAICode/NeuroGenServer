 ...
 * Core UI Module
 * 
 * Provides centralized UI functionality for the NeuroGen Processor system.
 * Enhanced with performance optimizations, improved error handling,
 * DOM safety checks, and better integration with core modules.
 * 
 * @module ui
 * @version 2.0.0
 */

// Import only what we need from uiRegistry to prevent conflicts
import { registerElement } from '../core/uiRegistry.js';

// WeakMap and Symbol for private state management (better memory efficiency)
...
/**
 * Ensures DOM is ready before executing operations that require DOM access
...
function domReady(callback) {
  ...
  if (state.documentReady || document.readyState !== 'loading') {
...
/**
 * Safe element accessor - uses uiRegistry when available or falls back to direct DOM access
...
function safeGetElement(selector) {
  try {
    // First check if uiRegistry is available and has getElement
    if (window.uiRegistry && typeof window.uiRegistry.getElement === 'function') {
      return window.uiRegistry.getElement(selector);
    }
...
const ui = {
  ...
  async initialize(options = {}) {
    ...
    try {
      ...
      // Publish to window for module access
      if (!window.ui) {
        window.ui = this;
      }
      ...
      // Try to register a minimal fallback version
      try {
        if (typeof registerElement === 'function') {
          const fallbackUI = {
          ...
          registerElement('ui', fallbackUI);
        }
      } catch (e) {
        console.error("Failed to register fallback UI module:", e);
      }
  ...
  _setupThemeObserver() {
    try {
      ...
      state.themeObserver.observe(document.documentElement, { 
        attributes: true,
        attributeFilter: ['data-theme', 'class', 'data-bs-theme']
      });
      ...
      if (document.body) {
        ...
        // If body isn't available, set up observer when it is
        document.addEventListener('DOMContentLoaded', () => {
          state.themeObserver.observe(document.body, {
            attributes: true, 
            attributeFilter: ['data-theme', 'class', 'data-bs-theme']
  ...
  _handleThemeChange() {
    try {
      ...
      const isDark = htmlTheme === 'dark' || bodyTheme === 'dark' || 
                    htmlBsTheme === 'dark' || bodyBsTheme === 'dark' ||
  ...
  _setupGlobalHandlers() {
    try {
      ...
      this._addTrackedEventListener(document, 'click', (event) => {
        // Check if the clicked element or any parent has data-action
          ...
          if (target.hasAttribute('data-action')) {
            ...
            switch (action) {
              case 'toggle-visibility':
                if (actionTarget) {
                  const el = safeGetElement(actionTarget);
                  if (el) {
                    const isVisible = el.style.display !== 'none';
                    this.toggleElementVisibility(actionTarget, !isVisible);
                  }
                }
                break;
  ...
  /**
   * Safe element getter that uses uiRegistry or direct DOM access
   * Provides centralized element retrieval to avoid redundant code
   * @param {string} selector - CSS selector or element ID
   * @returns {HTMLElement} Found element or null
   */
  getElement(selector) {
    return safeGetElement(selector);
  },
  ...
  ensureToastContainer() {
    ...
    try {
      ...
      if (document.body) {
        ...
        // If body isn't available yet, wait for DOM ready
        domReady(() => {
          document.body.appendChild(newContainer);
        });
      }
      ...
      try {
        ...
        console.error("Critical error creating toast container:", e);
        return null;
      }
    }
  },
   ...
   * @param {Object|number} options - Additional options or duration in ms
  ...
  showToast(title, message, type = 'info', options = {}) {
    try {
      ...
      // Check for Bootstrap Toast integration
      if (window.bootstrap && window.bootstrap.Toast) {
        try {
          // Initialize native Bootstrap toast features if available
          new window.bootstrap.Toast(toast);
        } catch (e) {
          console.warn("Error initializing Bootstrap Toast:", e);
        }
      }
   ...
   * @param {Object|boolean} options - Spinner options or fullscreen flag
  ...
  showLoadingSpinner(message = 'Loading...', options = {}) {
    try {
      ...
      // Add to document - with safety checks
      ...
      // If DOM not available, wait for it
      if (!addedToDOM) {
        domReady(() => {
          if (containerId) {
            const container = safeGetElement(containerId);
      ...
      const cancelButton = safeGetElement(`${spinnerId}-cancel`);
      ...
      return {
        ...
        updateMessage: (newMessage) => {
          const messageElement = safeGetElement(`${spinnerId}-message`);
          if (messageElement) {
  ...
  showModal(title, content, options = {}) {
    try {
      ...
      // Handle non-string content (DOM elements)
      ...
      if (window.bootstrap && window.bootstrap.Modal) {
        ...
        // For our manual implementation, add close button handlers
        const closeBtn = modal.querySelector('[data-bs-dismiss="modal"]');
        if (closeBtn) {
          this._addTrackedEventListener(closeBtn, 'click', () => {
            this.hideModal(modalId);
          });
        }
      }
   ...
   * @param {Object|string} options - Additional options or text to display
   * @returns {boolean} Success status
   */
  updateProgressBar(elementId, progress, options = {}) {
    try {
      ...
      const progressBar = safeGetElement(elementId);
          ...
          },
          {
            ...
            onClick: () => {
              ...
              if (required && (!value || value.trim() === '')) {
                ...
                const errorEl = safeGetElement(errorId);
  ...
  createCollapsible(triggerId, targetId, options = {}) {
    try {
      ...
      let triggerEl = safeGetElement(triggerId);
      let targetEl = safeGetElement(targetId);
      ...
      if (!triggerEl) {
        ...
        const placeholder = safeGetElement(`${triggerId}-placeholder`);
        if (placeholder) {
      ...
      if (!targetEl) {
        ...
        const placeholder = safeGetElement(`${targetId}-placeholder`);
        if (placeholder) {
...
// Automatically initialize the UI module on script load
domReady(() => {
  try {
    ui.initialize()
      .then(success => {
        if (!success) {
          console.warn("UI module did not initialize successfully");
        }
      })
      .catch(error => {
        console.error("Error during UI module initialization:", error);
      });
  } catch (e) {
    console.error("Critical error initializing UI module:", e);
  }
});
